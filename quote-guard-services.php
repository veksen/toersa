<?php
	...
	if(!empty($companyName) && !empty($evenType) && !empty($attendance) && !empty($hours) && !empty($demographic) && !empty($date) && !empty($alcohol) && !empty($email))
	{
		$mail = new PHPMailer();
		$mail->isSMTP();
		...
		$mail->addAddress('info@toersa.com', 'Guard Services');
		$mail->Subject = 'Quote for Guard Services';
